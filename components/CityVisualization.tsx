...
function PopulationVisualization({
  ...
  const people = Array.from({ length: displayCount }, (_, i) => {
    ...
    return (
      <div
        key={i}
        className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition transform hover:scale-125 ${
          isSick
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      <div className="rounded-lg border-2 border-blue-300 bg-blue-50 p-3">
        ...
        <p className={`text-xs font-bold ${
          housingSituation === 'Cukup' ? 'text-green-600' : 'text-red-600'
        }`}>
          {housingSituation}
        </p>
      </div>
